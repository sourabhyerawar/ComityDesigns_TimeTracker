/*
  	@ PURPOSE : CONTROLS THE LIGHTNING COMPONENT --> ManualTimeTrackerCmp.cmp
	@ NAME 	  : ManualTimeTrackerCmpController.js
*/
({
    /*
	...
	*/
	handleTabFocused : function( component, event, helper ) {      
        ...
        setTimeout( function() {          
            helper.handleTabFocused( component, helper );
        }, 2000 );
    },
	
	/*
		@ PURPOSE : FETCHES THE ID OF NEWLY CREATED RECORD WHEN THE RECORD IS SUCCESSFULLY SAVED.
    */
	...
	*/
	refreshSObjectDetails : function( component, event, helper ) {
        ...
        helper.refreshSObjectDetails( component, helper );
	},
	...
	*/
    ...
    /*
		@ PURPOSE : VALIDATES ACTIVITY TIME ON BLUR AND DISABLES SUBMIT BUTTON ON INVALID VALUE. 
	*/
    validateTime : function( component, event, helper ) {
		
        var activityDateElement = event.getSource();
    	if( activityDateElement ) {
            var consoleState = JSON.parse( JSON.stringify( component.get( "v.consoleState" ) ) );
            consoleState.toggleStatus.isSubmitDisabled = !( activityDateElement.reportValidity() && 
															helper.validateForm( component, helper, consoleState, false ) 
														  );
            component.set( "v.consoleState", consoleState );  
        }
    },
	...
	*/
	clearSearchtext : function( component, event, helper ) {
        ...
        helper.clearSearchtext( component, helper );
        var consoleState = JSON.parse( JSON.stringify( component.get( "v.consoleState" ) ) );
	...
	*/
	...
	/*
		@ PURPOSE : VALIDATES COMMENT FIELD CONTENTS AND DISPLAYS AN ERROR MESSAGE IF LEFT EMPTY WHEN IT IS REQUIRED. 
	*/
