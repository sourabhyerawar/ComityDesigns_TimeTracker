/*
  	@ PURPOSE : PROVIDES SERVICE TO THE LIGHTNING COMPONENT CONTROLLER --> ManualTimeTrackerCmpController.js
	@ NAME 	  : ManualTimeTrackerCmpHelper.js
*/
({	
    ...
    */
    initializeFooterComponent : function( component, helper ) {
		...
		helper.fetchManualTimeTrackerCmpWrapper( component, helper );
	},
	...
	*/
	handleTabFocused : function( component, helper ) {      
	...
	*/
	refreshSObjectDetails : function( component, helper ) {
        
        helper.clearValidationErrorMessages( component, helper );
		...
		if( consoleState && consoleState.toggleStatus && consoleState.typeAheadStatus ) {
			...
			if( consoleState.timeTracker && consoleState.timeTracker.activityName &&
				...
				if( matchedActivityWrapper ) {
					...
					consoleState.typeAheadStatus.sObjectSearchFieldLabel = matchedActivityWrapper.sObjectSearchFieldLabel;
				...
				consoleState.typeAheadStatus.sObjectSearchFieldLabel = "Case Number";
	...
	*/
	stop : function( component, helper ) { 
	...
	*/
	clearSearchtext : function( component, helper ) {
	...
	*/
				...
				if( !consoleState.timeTracker.objectId ) {
					...
					if( consoleState.typeAheadStatus.searchInput ) {
						...
						if( showError ) {
							...
							consoleState.toggleStatus.isInvalidRecordValueFound = true;
						}
					}
						...
						consoleState.toggleStatus.isInvalidRecordValueFound = false;
					}
				}
				else {
					recordSearchElement.set( "v.errors", [] );
	...
	/*
		@ PURPOSE : VALIDATES COMMENT FIELD CONTENTS AND DISPLAYS AN ERROR MESSAGE 
        			IF LEFT EMPTY WHEN IT IS RRQUIRED AND DISABLES SUBMIT BUTTON.
	*/
    ...
    /*
		@ PURPOSE : VALIDATES THE FORM AND ON FAILURE, DISPLAYS ERROR MESSAGES, DISABLES SUBMIT BUTTON AND ON SUCCESS, 
	...
	*/
    ...
    /*
		@ PURPOSE : 1. RECEIVES RECORD, LIST OF FIELDS.
					2. BUILDS LABEL WITH VALUES OF EACH FIELD SEPARATED BY " - ".
					2. TRIM THE CONTENTS TO AVOID THE X-SCROLLING.
	*/
	getLabelFromRecordAndFields : function( record, consoleState ) {
			
		var label = "";
		if( record && consoleState && consoleState.typeAheadStatus &&
			consoleState.typeAheadStatus.listOfShowFields && consoleState.typeAheadStatus.listOfShowFields.length > 0 
		) {
			
			if( !consoleState.typeAheadStatus.maxShowResultLength ) {
				consoleState.typeAheadStatus.maxShowResultLength = 45;
			}
			
			var separator = "";
			consoleState.typeAheadStatus.listOfShowFields.forEach( function( fieldApiName ) {
			...
			if( label.length > consoleState.typeAheadStatus.maxShowResultLength ) {
				label = label.substring( 0, consoleState.typeAheadStatus.maxShowResultLength ) + "...";
			}
		}
		return label;
	},
							...
							"typeAheadStatus" :
											   {
                                                ...
                                                "sObjectSearchFieldLabel" : "Case Number",
                                                "listOfShowFields" : [], "minimumSearchKeywordLength" : 1,
												"maxShowResultLength" : 45
											   },
							"toggleStatus" :
											{
                                             ...
                                             "isSobjectDisabled" : true, "isInvalidRecordValueFound" : false
											},
							"labelStatus" :
											{
	...
	*/
    ...
    /*
    	@ PURPOSE : 1. RE-RENDERS THE ACTIVITY DATE AND COMMENT FIELD SECTIONS.
        			2. HIDES THE STANDARD ERROR MESSAGES WHEN THEIR VALUES ARE AUTO-POPULATED.
    */
    clearValidationErrorMessages : function( component, helper ) {
    	
        component.set( "v.clearErrorMessage", false );
        component.set( "v.clearErrorMessage", true );
    },
	...
	*/
	fetchManualTimeTrackerCmpWrapper : function( component, helper ) {
	...
	*/
	searchRecords : function( component, helper, consoleState, getFromFocusedTab ) {
		...
		if( consoleState && consoleState.typeAheadStatus && consoleState.typeAheadStatus.searchInput &&
			consoleState.typeAheadStatus.searchInput.length > consoleState.typeAheadStatus.minimumSearchKeywordLength
		) {
			var constants = JSON.parse( JSON.stringify( component.get( "v.constants" ) ) );
			if( constants ) {
								...
								if( consoleState && consoleState.typeAheadStatus && 
									...
									if( getFromFocusedTab ) {
										...
										listOfMatchingRecords.forEach( function( matchingRecord ) {
											label = helper.getLabelFromRecordAndFields( matchingRecord, consoleState );
